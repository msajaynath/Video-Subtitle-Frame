...
final class Merge {
    ...
    func overlayVideo(video: AVAsset,
     ...
     //   let audioTrack = audioTracks.isEmpty ? NSNull : audioTracks[0]
...
fileprivate final class VideoComposition {
    ...
    init(size: CGSize, instruction: Instruction, frameRate: Int32, layer: Layer) {
      //  composition.renderSize = size
        composition.renderSize = CGSize(width: size.width, height: round(size.height+50))
...
fileprivate final class Layer {
    ...
    lazy var videoAndParent: VideoAndParent = {
        ...
        let videoLayerRect=CGRect(x: 0, y: 45, width: self.size.width, height: self.size.height)
        videoLayer.frame = videoLayerRect
        //videoLayer.borderWidth=5
       /// videoFrameLayer.borderColor=UIColor.red.cgColor
        
       /// let borderspace=2
        let videoFrameLayer = CALayer()
        videoFrameLayer.frame = self.frame
        let videoFrameLayerRect=CGRect(x: 0, y: 48, width: videoLayerRect.width+2, height: videoLayerRect.height)
        videoFrameLayer.frame = videoFrameLayerRect
        videoFrameLayer.borderColor=UIColor.red.cgColor
        videoFrameLayer.borderWidth=3
      ...
      //  parentLayer.frame=CGRect(x: 200, y: 200, width: self.size.width, height: 1000)
        parentLayer.addSublayer(videoLayer)
        parentLayer.addSublayer(videoFrameLayer)

       let titleCALayer = CALayer()
        let selfCARect = CGRect(x: 0, y: 0 , width: self.size.width, height: 45)
        titleCALayer.frame = selfCARect
        
        titleCALayer.contents=self.overlay.cgImage
        parentLayer.addSublayer(titleCALayer)
